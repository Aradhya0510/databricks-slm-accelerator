...
with c1:
    metric_card("Tracked Endpoints", str(len(endpoints)))
with c2:
    metric_card("Error Threshold", f"{mon.get('error_rate_threshold', 0.05):.0%}")
with c3:
    metric_card("Latency P95 Limit", f"{mon.get('latency_p95_threshold_ms', 2000)}ms")
...
with tab_health:
    ...
    if st.button("Check Health", type="primary") and selected_endpoint:
        try:
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.markdown(f"**Endpoint:** `{ep_status['endpoint_name']}`")
                st.markdown(
                    f"**Ready:** {status_badge(ep_status.get('ready', 'UNKNOWN'))}",
                    unsafe_allow_html=True,
                )
            with c2:
                for model in ep_status.get("served_models", []):
                    st.markdown(
                        f"**Model:** {model.get('entity_name', 'N/A')} "
                        f"v{model.get('entity_version', '?')}"
                    )
                    st.markdown(
                        f"**GPU:** {model.get('workload_type', 'N/A')}"
                    )
    ...
    st.info(
        "Run this query in the Databricks SQL Editor or via the SDK to see metrics."
    )
...
with tab_settings:
    ...
    with c1:
        drift_threshold = st.number_input(
            ...
            min_value=0.0,
            max_value=1.0,
            value=mon.get("drift_threshold", 0.1),
            step=0.01,
        )
    with c2:
        error_threshold = st.number_input(
            ...
            min_value=0.0,
            max_value=1.0,
            value=mon.get("error_rate_threshold", 0.05),
            step=0.01,
        )
    with c3:
        latency_threshold = st.number_input(
            ...
            min_value=100,
            max_value=30000,
            value=mon.get("latency_p95_threshold_ms", 2000),
            step=100,
        )
    ...
    with c1:
        catalog = st.text_input(
            "Default Catalog", value=prefs.get("default_catalog", "main")
        )
        schema = st.text_input(
            "Default Schema", value=prefs.get("default_schema", "slm_models")
        )
    with c2:
        volume = st.text_input(
            "Default Volume", value=prefs.get("default_volume", "slm_data")
        )
        email = st.text_input(
            "Workspace Email", value=prefs.get("workspace_email") or ""
        )
